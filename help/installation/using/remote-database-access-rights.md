...
* **MAKEN/NEERZETTEN** for **TABEL/INDEX/PROCEDURE/FUNCTIE** (alleen voor door Adobe Campaign gegenereerde werktabellen),
...
|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
| **Connecting to remote database** | GEBRUIK OP WAREHOUSE, GEBRUIK OP DATABASE EN GEBRUIK OP SCHEMA-privileges | Een gebruiker maken die is gekoppeld aan de AWS-account | SESSIEBEVOEGDHEID MAKEN | machtiging CONNECT | CONNECT-bevoegdheid | Een gebruiker maken die is gekoppeld aan een externe host die ALLE PRIVILEGES heeft |
...
| **Loading data into tables** | WERKGEBIED MAKEN OP SCHEMA, SELECTEREN en INVOEGEN in de rechten voor de doeltabel | SELECTEREN EN INVOEGEN, rechten | SELECTEREN EN INVOEGEN, rechten | BULKBEWERKINGEN INVOEGEN, BEHEREN en TABELmachtigingen WIJZIGEN | SELECTEREN EN INVOEGEN, rechten | BESTANDSBEVOEGDHEID |
| **Toegang tot clientgegevens** | SELECTEREN OP (TOEKOMSTIGE) TABLE(S)- OF WEERGAVEBEVOEGDHEDEN | SELECT, bevoegdheid | SELECTEER OF SELECTEER EEN TABELBEVOEGDHEID | SELECT permission | SELECT, bevoegdheid | SELECT, bevoegdheid |
| **Toegang tot metagegevens** | SELECT on INFORMATION_SCHEMA SCHEMA privilege | SELECT, bevoegdheid | Geen bevoegdheid vereist voor gebruik van DESCRIBE-instructie | machtiging DEFINITIE WEERGEVEN | Geen bevoegdheid vereist voor het gebruik van de opdracht &quot;\d table&quot; | SELECT, bevoegdheid |
...
| **Creating procedures** | IMPLICIT_SCHEMA authority or CREATEIN privilege | CREATE PROCEDURE or PROCEDURE keyword | CREATE ROUTINE privilege | RESOURCE authority | PROCESBEVOEGDHEID | FUNCTIE-bevoegdheden MAKEN |
...
| **Uitvoeringen controleren** | EXPLAIN-instantie | No privilege required to use EXPLAIN statement | SELECT, bevoegdheid | Alleen een systeembeheerder kan sp_showplan uitvoeren | No privilege required to use EXPLAIN statement | Geen bevoegdheid vereist om instructie EXPLAIN te gebruiken |
| **Gegevens schrijven** | Rechten of DATAACCESS-instantie INVOEGEN en bijwerken | INSERT and UPDATE privileges | Rechten INVOEGEN en BIJWERKEN | INSERT and UPDATE permissions | Rechten INVOEGEN en BIJWERKEN | Rechten INVOEGEN en BIJWERKEN |
| **Gegevens in tabellen laden** | LAADINSTANTIE | U kunt privileges SELECTEREN en INVOEGEN om respectievelijk KOPIËREN NAAR en KOPIËREN UIT instructies te gebruiken | BESTANDSBEVOEGDHEID | Ben de eigenaar van de lijst of ALTER toestemming. Depending on the -gl option, LOAD TABLE might only be performed if the user has the DBA authority | SELECTEREN EN INVOEGEN, rechten | SELECTEREN EN INVOEGEN, rechten |
