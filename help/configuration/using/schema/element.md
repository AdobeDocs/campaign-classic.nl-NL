...
* **dataPolicy (string)**: kunt u goedkeuringsbeperkingen opgeven voor waarden die zijn toegestaan in het SQL-veld. De waarden voor dit kenmerk zijn:
...
* **tableSpace (tekenreeks)**: Met dit kenmerk kunt u een nieuwe gegevensopslagruimte opgeven voor een tabel (geldig in het basiselement) `<element>`).
* **tableSpaceIndex (tekenreeks)**: Met dit kenmerk kunt u een nieuwe tabelruimte voor indexopslag voor een tabel opgeven (geldig in het hoofdgedeelte) `<element>`).
...
* **sjabloon (tekenreeks)**: this attribute define a reference to an `<element>` element dat door verscheidene schema&#39;s wordt gedeeld. De definitie wordt automatisch gekopieerd naar het huidige schema.
...
* **translateExpr (tekenreeks)**: als een &quot;@expr&quot;attribuut wordt gevonden, laat &quot;@translateExpr&quot;attribuut u een uitdrukking opnieuw bepalen die in &quot;@expr&quot;wordt bepaald, en die door het vertaalhulpmiddel (intern gebruik) zal worden verzameld.
...
* **unbound (boolean)**: als het attribuut wordt geactiveerd (unbound= &quot;waar&quot;), wordt de verbinding verklaard als inzamelingselement voor een kardinaliteit 1-N.
